...
SCRIPT_COUNT=0
for script in "$HARNESS_DIR"/scripts/*.sh; do
    if [ -f "$script" ]; then
...
for script in "$HARNESS_DIR"/scripts/*.py; do
    if [ -f "$script" ]; then
...
if ! python3 "$HARNESS_DIR/scripts/harness-status.py" "$HARNESS_DIR" > /dev/null 2>&1; then
...
TPL_COUNT=0
for tpl in "$HARNESS_DIR"/templates/feature/*.md "$HARNESS_DIR"/templates/big-picture/*.md; do
    if [ -f "$tpl" ]; then
...
if ! grep -qE "^# --- MANAGED_SCAFFOLD_START" "$HARNESS_DIR/scripts/sanity-check.sh" 2>/dev/null; then
    echo "❌ FAIL: scripts/sanity-check.sh missing MANAGED_SCAFFOLD_START boundary marker"
    ERRORS=$((ERRORS + 1))
fi
if ! grep -qE "^# --- MANAGED_SCAFFOLD_END" "$HARNESS_DIR/scripts/sanity-check.sh" 2>/dev/null; then
...
if [ -f "$HARNESS_DIR/scripts/test-harness.sh" ]; then
...
if [ -f "$HARNESS_DIR/PHASES.md" ]; then
    if head -5 "$HARNESS_DIR/PHASES.md" | grep -q "Template"; then
...
if grep -rl "templates/PHASES\.md" "$HARNESS_DIR"/commands/ 2>/dev/null | xargs grep -L "no longer exists\|Do not create" 2>/dev/null | grep -q .; then
    echo "❌ FAIL: positive reference to obsolete templates/PHASES.md found in commands/"
    ERRORS=$((ERRORS + 1))
fi
if grep -rl "templates/sanity-check\.sh" "$HARNESS_DIR"/commands/ 2>/dev/null | xargs grep -L "no longer exists\|Do not create" 2>/dev/null | grep -q .; then
...
if ! bash -n "$HARNESS_DIR/hooks/pre-commit" 2>/dev/null; then
...
DOC_COUNT=0
for doc in "$HARNESS_DIR"/docs/*.md; do
    if [ -f "$doc" ]; then
